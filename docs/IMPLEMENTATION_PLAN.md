...
WP5 ──▶ WP10a (session core, pure) ──┬──▶ WP10b (host session driver + UX)
                                     └──▶ WP10c (keyboard live mic key)
```
...
## WP10 — Dictation Sessions (keyboard-live dictation)

> Architecture: [ARCHITECTURE.md](ARCHITECTURE.md) §5 flow 1.5, §6.8, D11/D12,
> risks R10a–c. The competitive gap this closes: competitors amortize ONE
> app-hop across an armed background-mic window and stream partials into the
> keyboard live; we currently pay the hop per dictation and insert only finals.

### WP10a — Session core (pure; MobileCore + KeyboardCore)

**Repo:** `openwhisp-ios`. **Blocked by:** WP5 (shipped). **Blocks:** WP10b, WP10c.

- `MobileCore`: `DictationSessionConfig`, `SessionStatus` (+ staleness rule),
  `SessionCommand`, `SessionCommandMailbox` protocol + App Group file impl
  (same O_EXCL claim-rename discipline as `AppGroupHandoffStore`, 5 s command
  expiry) + in-memory test double; `LivePartial`, `LivePartialStore` protocol +
  last-writer-wins file impl + double; `SessionFlow` state machine exactly per
  §6.8 (every event × phase tested, incl. idle-timeout edges, stale commands,
  interruption teardown, `never` timeout).
- `KeyboardCore`: `LiveInsertDiffer` (prefix growth, mid-string revision,
  shrink, final swap, empty, emoji/combining-character safety); extend
  `MicKeyResolver` with session-aware behaviors (`.startSessionHop`,
  `.startCapture`, `.stopCapture`) driven by `SessionStatus` + staleness —
  full truth table updated.
- Darwin names (constants in MobileCore): `app.openwhisp.session.command`,
  `app.openwhisp.session.partial`, `app.openwhisp.session.status`.

**Acceptance:** `swift test` green; no UI, no OS-bound code beyond the App
Group file stores (tempdir-tested like the handoff store).

### WP10b — Host session driver + arming UX

**Repo:** `openwhisp-ios`. **Blocked by:** WP10a.

- `CaptureKit`: `SessionHolder` (@MainActor) per §6.8 driver notes — audio
  session keep-alive under the `audio` background mode, Darwin listener + 250 ms
  mailbox poll while armed, capture via existing `CaptureCoordinator`
  (`.keyboardHandoff`), partial publishing throttled ≤ 8/s, heartbeat status
  writes, idle-timeout + interruption + termination teardown.
- Host app: arming screen ("Session on — swipe back to your app"), deep link
  `openwhisp://session/arm` in `DeepLink`, Settings idle-timeout picker +
  privacy copy, Live Activity armed/capturing states + End Session button.
- Device spike checklist for R10a (armed survival: locked / other app
  foreground / Low Power Mode; battery over a 15-min armed window) — manual
  checklist in TESTING.md, not CI.

**Acceptance:** package tests green + simulator build; state-machine drivers
covered by tests with fake stores; R10a checklist written (execution is the
human's real-device pass).

### WP10c — Keyboard: live mic key + partial rendering

**Repo:** `openwhisp-ios`. **Blocked by:** WP10a (NOT by 10b — build against
the stores/doubles).

- Mic key drives the new `MicKeyResolver` behaviors: no session → today's
  floor-flow hop (unchanged); armed → post `startCapture`/`stopCapture` to the
  mailbox, render capture state (pulse while capturing).
- Partial rendering loop: Darwin ping + 250 ms poll while capturing; apply
  `LiveInsertDiffer` edits through `ProxyTextSink`; final (`isFinal`) swaps in
  cleaned text and clears; NEVER render into `isSecureField` (fall back to the
  WP5 pending-transcript path); Full Access off → session features invisible.
- Keyboard stays a functional keyboard with all of this off [C8]; typing
  never blocks on session state.

**Acceptance:** `swift test` green (resolver truth table, differ), simulator
build; XCUITest smoke where feasible (mic key states with a seeded status file).
