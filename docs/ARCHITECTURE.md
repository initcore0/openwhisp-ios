...
| **D5** | **Transcription engine: WhisperKit (CoreML/ANE), models `tiny`/`base`/`small`; Apple `SpeechAnalyzer` (iOS 26) as a second engine behind the same protocol** | WhisperKit is iOS-native and already integrated upstream. The engine seam (`StreamingTranscriptionEngine`) already exists, so an Apple-Speech engine is a bounded later add (ticket explicitly allows either). Parakeet (macOS PR #163) can slot in the same way once merged upstream. |
